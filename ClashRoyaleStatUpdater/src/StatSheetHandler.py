...
class StatManager(DataExtractor):
    ...
    def _insert_missing_data(self, statistics: list):
        ...
        if len(new_stats) > 0:
            ...
            _participation_range = f'F2:{last_col_index}5'
            _participation = [
                [x["participation_percentage"] for x in new_stats],
                [x["participation"] for x in new_stats],
                [x["points"] for x in new_stats],
                [f'={chr(x)}4/REGEXEXTRACT({chr(x)}3;"[0-9]+")' for x in range(70, 70+len(new_stats))]  # chr(70) == 'F'
            ]
    ...
    def update_statistics(self):
        try:
            ...
            self._insert_missing_data(statistics)
